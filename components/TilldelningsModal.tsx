...
import type { OtilldeladFunktionar, PassMedSektion } from '@/lib/database.types'

interface Props {
  // Förvälj antingen en funktionär eller ett pass
  valtFunktionar?: OtilldeladFunktionar
  valtPassId?: string
  // Tillgängliga val
...
export default function TilldelningsModal({
  ...
  const valdFunktionarInfo = otilldelade.find((f) => f.id === valdFunktionar)
    ?? (valtFunktionar?.id === valdFunktionar ? valtFunktionar : undefined)
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={hanteraSubmit} className="px-6 py-5 space-y-5">
          ...
          <div>
            ...
            {valtFunktionar ? (
              // Förvald — visa som kort
              <div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3">
                <p className="text-sm font-semibold text-gray-900">
                  {valtFunktionar.full_name ?? '(inget namn)'}
                </p>
          ...
          <div>
            ...
            {valtPassId ? (
              // Förvalt pass — visa som kort
              valdPassInfo && (
                <div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3">
                  <p className="text-sm font-semibold text-gray-900">{valdPassInfo.sektion_namn}</p>
                  <p className="text-xs text-gray-600">
                    {valdPassInfo.pass_namn} · {valdPassInfo.starttid}–{valdPassInfo.sluttid}
                  </p>
                  <p className="text-xs text-gray-400 mt-1">
                    {valdPassInfo.tilldelade}/{valdPassInfo.behovs_antal} tilldelade
                  </p>
                </div>
              )
            ) : (
          ...
          {/* Preferenser-info om funktionär är vald */}
          ...
          {/* Felmeddelande */}
          {fel && (
            <p className="text-sm text-red-600 bg-red-50 rounded-lg px-3 py-2">{fel}</p>
          )}
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button
              type="submit"
              disabled={pending || !valdFunktionar || !valdPass}
