-- Migration 013: Lägg till kompetenser på pass

ALTER TABLE public.pass
  ADD COLUMN IF NOT EXISTS kompetenser text[] DEFAULT '{}';

-- Uppdatera RPC get_pass_med_sektioner för att inkludera kompetenser
...
AS $$
  SELECT
    ...
    GREATEST(0, p.behovs_antal - COUNT(t.id) FILTER (WHERE t.status = 'bekraftad')) AS saknas,
    s.id                                           AS sektion_id,
